...
if "font_choice" not in st.session_state:
    st.session_state.font_choice = "🔹 標準"
...
# --- ⚖️ 貼心字體隨動縮放核心邏輯 ---
scale = 1.0
if st.session_state.font_choice == "🔎 放大":
    scale = 1.25  # 放大 1.25 倍
elif st.session_state.font_choice == "🚀 特大":
    scale = 1.50  # 放大 1.5 倍（專門照顧老醫師靈魂）

# 動態計算各元件像素大小
title_font = int(32 * scale)
subtitle_font = int(21 * scale)
expander_header_font = int(16 * scale)
body_font = int(14.5 * scale)
input_font = int(15 * scale)
table_font = int(14 * scale)
...
st.markdown(
    ...
    <div style="margin-bottom: 12px;">
        <p style="margin: 0; padding-bottom: 0px; font-size: {title_font}px; font-weight: bold; color: #ffffff; font-family: 'Microsoft JhengHei', sans-serif;">🌐 世衛&醫教主題會議捕手</p>
        <p style="margin: 8px 0 0 50px; font-size: {subtitle_font}px; font-weight: bold; color: #e2e8f0; font-family: 'Microsoft JhengHei', sans-serif; letter-spacing: 0.5px;">WHO & MedEd Thematic Conf Catcher</p>
    </div>
    """,
...
# 作者資訊與全新「長輩級字體切換鈕」欄位
col_meta1, col_meta2, col_meta3 = st.columns([1, 1.2, 1])
with col_meta1:
    st.caption("🔄 目前更新版本日期: 2026 / 05 / 25")
with col_meta2:
    # 讓老醫師一鍵點選的橫向大按鈕
    st.session_state.font_choice = st.radio(
        "💡 畫面字體大小調整（覺得字太小請點選放大）：",
        options=["🔹 標準", "🔎 放大", "🚀 特大"],
        index=["🔹 標準", "🔎 放大", "🚀 特大"].index(st.session_state.font_choice),
        horizontal=True
    )
with col_meta3:
    st.markdown("<p style='text-align: right; color: #868e96; font-size: 14px; margin: 0; padding-top: 25px;'>系統維護：魏今秀 (教學研究部 醫學教學科)</p>", unsafe_allow_html=True)


# 🎨 智慧三色調 CSS 注入（全面導入動態字級變數）
st.markdown(
    f"""
    <style>
    ...
    div[data-testid="stExpander"] summary p {{
        ...
        font-size: {expander_header_font}px !important; 
    ...
    /* 擴充：確保輸入框提示標籤、內部文字與下拉清單全面隨動放大 */
    div[data-testid="stTextInput"] label p, div[data-testid="stMultiSelect"] label p {{
        font-size: {body_font}px !important;
    }}
    div[data-testid="stTextInput"] input {{
        color: #f1f5f9 !important;
        font-size: {input_font}px !important;
    }}
    div[data-testid="stMultiSelect"] div[data-orientable="true"] {{
        font-size: {input_font}px !important;
    }}
    
    /* 全域核心：讓 Streamlit 核心表格 (DataFrame) 內的所有文字完美隨動放大 */
    .stDataFrame div, div[data-testid="stDataFrame"] {{
        font-size: {table_font}px !important;
    }}
    
    /* 系統內建下載按鈕與一般純文字段落隨動放大 */
